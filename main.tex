...
\vspace{5mm}
Let $m(\geq 2)$ be an initial or pilot sample size, not involving $d$. In the case of each estimation strategy that will be discussed later , we will readily see that the boundary conditions for stopping will depend only upon the sample variance(s), that is the event $[N=n]$ would only depend on $(S_m^2,\dots,S_n^2)$ for all $n \geq m$, ultimately arriving at the terminally accrued data, 
